...
import { Avatar, AvatarFallback, AvatarImageNext } from "@unstage/ui/components/avatar";
...
export function TeamDropdown({ isExpanded = false }: Props) {
  ...
  const [selectedId, setSelectedId] = useState<string | null | undefined>(user?.organizationId);
  ...
  const { data: teams } = useQuery(trpc.organization.list.queryOptions());
  ...
  return (
    <div className="relative h-[32px]" ref={ref}>
            ...
            {isActive && (
              ...
              >
                <Link href="/teams/create" onClick={() => setActive(false)}>
                  <Button className="w-[32px] h-[32px]" size="icon" variant="outline">
                    <Icons.Add />
                  </Button>
                </Link>
              </motion.div>
      ...
      {sortedTeams[0] && (
        <div
          className={cn(
            "fixed bottom-4 h-[32px] flex items-center transition-all duration-200 ease-in-out",
            isExpanded ? "left-[62px] opacity-100" : "left-[30px] opacity-0"
          )}
        >
          <button
            type="button"
